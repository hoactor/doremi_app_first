...
export const AssetLibraryModal: React.FC<AssetLibraryModalProps> = ({
  ...
  const handleDownload = async (e: React.MouseEvent, asset: LibraryAsset) => {
      e.stopPropagation();
      const fileName = `${asset.source.name.replace(/\s+/g, '_')}_${asset.id.substring(0, 4)}.png`;
      try {
          const resp = await fetch(asset.imageDataUrl);
          const blob = await resp.blob();
          await downloadFile(blob, fileName, [{ name: 'PNG Image', extensions: ['png'] }]);
      } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[100] p-4 animate-fade-in">
      <div className="bg-zinc-800 rounded-2xl shadow-xl w-full max-w-6xl h-[90vh] flex flex-col">
        ...
        <div className="p-4 border-b border-zinc-700 flex justify-between items-center gap-4">
            <input
                type="text"
                placeholder="태그(예: 인물, 배경), 프롬프트, 출처로 검색..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full p-2 bg-zinc-700 rounded-md text-sm border border-zinc-600 focus:ring-orange-500"
            />
            <div className="flex items-center gap-2 flex-shrink-0">
                <button onClick={() => setCategoryFilter('all')} disabled={mode !== 'normal'} className={`px-3 py-1 text-sm rounded-full ${categoryFilter === 'all' ? 'bg-orange-600 text-white' : 'bg-zinc-600 hover:bg-zinc-500'} ${mode !== 'normal' ? 'cursor-not-allowed opacity-50' : ''}`}>전체</button>
                <button onClick={() => setCategoryFilter('character')} disabled={mode !== 'normal'} className={`px-3 py-1 text-sm rounded-full ${categoryFilter === 'character' ? 'bg-orange-600 text-white' : 'bg-zinc-600 hover:bg-zinc-500'} ${mode !== 'normal' ? 'cursor-not-allowed opacity-50' : ''}`}>인물</button>
                <button onClick={() => setCategoryFilter('background')} disabled={mode !== 'normal'} className={`px-3 py-1 text-sm rounded-full ${categoryFilter === 'background' ? 'bg-orange-600 text-white' : 'bg-zinc-600 hover:bg-zinc-500'} ${mode !== 'normal' ? 'cursor-not-allowed opacity-50' : ''}`}>배경</button>
                <button onClick={handleImportClick} className="inline-flex items-center px-3 py-1 text-sm rounded-full bg-zinc-700 text-white hover:bg-zinc-700">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              {filteredAssets.map(asset => (
                <div key={asset.id} className="group relative" onClick={() => onSelect(asset)}>
                  <div className="aspect-square w-full bg-zinc-700 rounded-lg overflow-hidden">
                    <img
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity p-2 flex flex-col justify-end text-white text-xs rounded-lg">
                     ...
                     <p className="text-gray-300">출처: {asset.source.name}</p>
                  </div>
                  <button 
