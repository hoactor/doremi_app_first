    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 animate-fade-in" aria-modal="true" role="dialog">
      <div className="bg-zinc-800 rounded-2xl shadow-xl w-full max-w-4xl h-[80vh] flex flex-col transform transition-all duration-300 scale-95 opacity-0 animate-fade-in-scale">
        ...
        <div className="flex flex-grow overflow-hidden">
          <div className="w-2/3 p-4 overflow-y-auto border-r border-zinc-700">
            <h3 className="font-semibold text-zinc-300 mb-2">
              {isReplacementMode ? '저장된 인물 (클릭하여 선택)' : '저장된 인물'}
            </h3>
            {characters.length === 0 ? (
                <p className="text-zinc-400 text-center mt-8">라이브러리가 비어있습니다. 우측 패널에서 새 인물을 업로드하세요.</p>
            ) : (
                ...
                {characters.map(char => (
                    <div key={char.id} className="group relative">
                        ...
                        />
                        <p className="text-xs text-center mt-1 truncate text-zinc-300">{char.name}</p>
                        <button 
          ...
          <div className="w-1/3 p-4 bg-zinc-800/50 flex flex-col">
            <div className="space-y-4">
                <h3 className="font-semibold text-zinc-300">파일에서 새 인물 추가</h3>
                <div 
                    onClick={handleUploadClick}
                    className="w-full aspect-square bg-zinc-700 rounded-lg flex items-center justify-center cursor-pointer border-2 border-dashed border-zinc-500 hover:border-orange-500"
                >
                    {newCharacterImage ? (
                ...
                <div>
                    <label htmlFor="char-name" className="block text-sm font-medium text-zinc-300 mb-1">인물 이름</label>
                    <input
                        id="char-name"
                        type="text"
                        value={newCharacterName}
                        onChange={(e) => setNewCharacterName(e.target.value)}
                        className="w-full p-2 bg-zinc-700 rounded-md text-sm text-zinc-200 border border-zinc-500 focus:ring-orange-500 focus:border-orange-500"
