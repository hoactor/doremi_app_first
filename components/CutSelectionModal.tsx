  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-[90] flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-stone-800 border border-stone-700 rounded-2xl shadow-xl w-full max-w-2xl h-[85vh] flex flex-col">
        ...
        <div className="p-4 flex-shrink-0 flex justify-between items-center border-b border-stone-700">
            <p className="text-sm text-stone-400">{selectedCuts.length} / {allCuts.length}개 컷 선택됨</p>
            <div className="flex gap-2">
                <button onClick={handleSelectAll} className="px-3 py-1 text-xs font-semibold rounded-md bg-stone-600 hover:bg-stone-500">전체 선택</button>
                <button onClick={handleDeselectAll} className="px-3 py-1 text-xs font-semibold rounded-md bg-stone-600 hover:bg-stone-500">전체 해제</button>
            </div>
        </div>
            ...
            <div key={scene.sceneNumber}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                    {(scene.cuts || []).filter(Boolean).map(cut => (
                        <label key={cut.cutNumber} className={`p-2 rounded-md text-center cursor-pointer transition-colors font-mono text-sm ${selectedCuts.includes(cut.cutNumber) ? 'bg-amber-600 text-white font-semibold' : 'bg-stone-800 text-stone-400 border border-stone-600 hover:bg-stone-700 hover:border-stone-500'}`} title={`Cut #${cut.cutNumber}: ${cut.narration}`}>
                            <input 
                                type="checkbox"
        ...
        <footer className="p-4 bg-stone-900/50 border-t border-stone-700 flex-shrink-0">
          <button
            onClick={handleConfirm}
            disabled={selectedCuts.length === 0}
            className="w-full flex items-center justify-center gap-2 px-6 py-3 text-base font-bold rounded-lg text-white bg-green-600 hover:bg-green-500 transition-colors disabled:opacity-50"
          >
